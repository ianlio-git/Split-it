    ...
    if (state === "Pending") {
      ...
      return (
        <span className="text-green-500 flex items-center justify-end">
          <FaCheckCircle className="mr-2 text-lg" /> {/* Símbolo de tilde */}
      ...
      <div className="max-w-3xl mx-auto space-y-4">
        {friends.length > 0 ? (
          friends.map((friend) => (
            <div
              key={friend.id}
              className="flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-4 bg-gray-700 p-4 rounded-xl"
            >
              <div className="w-12 h-12 bg-blue-500 rounded-full flex items-center justify-center">
                <span className="text-2xl font-bold">{friend.name[0]}</span>
              </div>
              <div className="flex-1">
                <h2 className="font-semibold text-white text-center sm:text-left">
                  {friend.name}
                </h2>
